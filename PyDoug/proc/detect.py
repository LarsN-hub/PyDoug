...
def blobs(
        im_array: np.ndarray, method: str = "dog", *,
        min_sigma: int = 1,
        max_sigma: int = 50,
    ...
    if method == "dog":
        ...
        blobs_coords: np.ndarray = feature.blob_dog(
            im_array,
            min_sigma = min_sigma,
            max_sigma = max_sigma,
        ...
        if im_array.ndim > 2:
            ...
            for slice_index in range(0, im_array.shape[0]):
                ...
                current_coords: np.ndarray = feature.blob_doh(
                    im_array[slice_index],
                    min_sigma = min_sigma,
                    max_sigma = max_sigma,
                    num_sigma = num_sigma,
                    ...
                    log_scale = log_scale,
                    threshold_rel = threshold_rel)
                slice_append: np.ndarray = np.repeat(
                    np.expand_dims(np.array([slice_index]), axis = 1),
                    current_coords.shape[0],
                    axis = 0)
                current_coords = np.append(
                    slice_append,
                    current_coords,
                    axis = 1)
                blobs_coords = np.append(
                    blobs_coords,
                    current_coords,
                    axis = 0)
            ...
            blobs_coords: np.ndarray = feature.blob_doh(
                im_array,
                min_sigma = min_sigma,
                max_sigma = max_sigma,
                num_sigma = num_sigma,
        ...
        blobs_coords: np.ndarray = feature.blob_log(
            im_array,
            min_sigma = min_sigma,
            max_sigma = max_sigma,
            num_sigma = num_sigma,
