...
class ImageProcessor:
    ...
    def __init__(self, viewer: napari.viewer.Viewer) -> None:
        ...
        self.funcguis: dict[str, widgets.FunctionGui] = get_funcguis(
            ImageProcessor)
        ...
        self.manual_threshold_widget.Image.changed.connect(
            self._update_intensity_range)
        self.manual_threshold_widget.Range.changed.connect(
            self._live_threshold)
        self.manual_threshold_widget.Image.changed.connect(
            self._live_threshold)
        self.manual_threshold_widget.Preview.changed.connect(
    ...
    def split_widget(self,
        ...
        self.parameters_log.append(
            {"Name": param_layer_name,
             ...
             "Axis": Axis})
        split_arrays: list[np.ndarray] = cc.split(
            Image.data,
            Split_Index,
            Axis)
        self.viewer.add_image(
            split_arrays[0],
            name = f"{param_layer_name} - 1")
        self.viewer.add_image(
            split_arrays[1],
            name = f"{param_layer_name} - 2")
    ...
    def join_widget(self,
        ...
        self.parameters_log.append(
            {"Name": param_layer_name,
             "Acting On": Image_1.name,
             "Axis": Axis})
        self.viewer.add_image(
            cc.join(
                [Image_1.data, Image_2.data],
                Axis),
            name = param_layer_name)
    ...
    def mirror_widget(self,
        ...
        Direction = util.convert_ax_str_to_int(
            Image.data,
            Image.rgb,
            Direction)
        param_layer_name = get_param_layer_name(
    ...
    def mask_logic_widget(self,
        ...
        self.parameters_log.append(
            {"Name": param_layer_name,
             "Acting On": Mask_1.name,
             "Method": Method.lower(),
             "Apply Mask": True,
             "Mask Used": Mask_2.name,
             "Unique Masks": Unique_Batch_Masks})
        self.viewer.add_image(
            cc.mask_logic(
                Mask_1.data,
    ...
    def ring_removal_widget(self,
        ...
        Square_Axis = util.convert_ax_str_to_int(
            Image.data,
            Image.rgb,
            Square_Axis)
        param_layer_name = get_param_layer_name(
    ...
    def labels_2_image_widget(self,
        ...
        self.parameters_log.append(
            {"Name": param_layer_name,
             "Acting On": Labels.name})
        self.viewer.add_image(
            pixels.labels_2_rgb(Labels.data),
            name = param_layer_name)
    ...
    def image_2_labels_widget(self,
        ...
        Gradient_Axis = util.convert_ax_str_to_int(
            Image.data,
            Image.rgb,
            Gradient_Axis)
        param_layer_name = get_param_layer_name(
...
def get_param_layer_name(
        operation_name: str,
        operation_count: int) -> str:
    
    return f"{operation_name} [{operation_count}]"

def get_funcguis(
        magic_class) -> dict[str, widgets.FunctionGui]:
...
def main() -> napari.viewer.Viewer:
    ...
    mod_im_import: widgets.Container = modify_funcgui(
        ui.im_import_widget, "Import File")
    mod_dir_import: widgets.Container = modify_funcgui(
        ui.dir_import_widget, "Import File Sequence")
    mod_im_export: widgets.Container = modify_funcgui(
        ui.im_export_widget, "Export Image(s)")
    mod_param_export: widgets.Container = modify_funcgui(
        ui.export_parameters_widget, "Export Parameters")
    mod_lab_export: widgets.Container = modify_funcgui(
        ui.lab_export_widget, "Export Labels")
    mod_screenshot: widgets.Container = modify_funcgui(
        ui.screenshot_widget, "Capture Screenshot")
    mod_batch: widgets.Container = modify_funcgui(
        ui.batch_widget, "Batch Processing")
    ...
    mod_trim_pad: widgets.Container = modify_funcgui(
        ui.trim_pad_widget, "Trim / Pad")
    mod_crop: widgets.Container = modify_funcgui(
        ui.crop_widget, "Crop")
    mod_split: widgets.Container = modify_funcgui(
        ui.split_widget, "Split")
    mod_join: widgets.Container = modify_funcgui(
        ui.join_widget, "Join")
    mod_extend: widgets.Container = modify_funcgui(
        ui.extend_widget, "Extend")
    ...
    mod_reslice: widgets.Container = modify_funcgui(
        ui.reslice_widget, "Reslice")
    mod_rotate: widgets.Container = modify_funcgui(
        ui.rotate_widget, "Rotate")
    mod_mirror: widgets.Container = modify_funcgui(
        ui.mirror_widget, "Mirror")
    mod_resize: widgets.Container = modify_funcgui(
        ui.resize_widget, "Resize")
    mod_rescale: widgets.Container = modify_funcgui(
        ui.rescale_widget, "Rescale")
    ...
    mod_mask: widgets.Container = modify_funcgui(
        ui.mask_widget, "Mask")
    mod_add_shape: widgets.Container = modify_funcgui(
        ui.add_shape_widget, "Add Shape")
    mod_create_shape_mask: widgets.Container = modify_funcgui(
        ui.create_shape_mask_widget, "Create Mask from Shapes")
    mod_paint: widgets.Container = modify_funcgui(
        ui.paint_widget, "Paint")
    mod_create_paint_mask: widgets.Container = modify_funcgui(
        ui.create_paint_mask_widget, "Create Mask from Paint")
    mod_mask_logic: widgets.Container = modify_funcgui(
        ui.mask_logic_widget, "Mask Logic Operations")
    ...
    mod_convert_type: widgets.Container = modify_funcgui(
        ui.convert_type_widget, "Convert Type")
    mod_normalize: widgets.Container = modify_funcgui(
        ui.normalize_widget, "Normalize")
    mod_saturate: widgets.Container = modify_funcgui(
        ui.saturate_widget, "Saturate")
    mod_equalize: widgets.Container = modify_funcgui(
        ui.equalize_widget, "Equalize Histogram")
    mod_reassign: widgets.Container = modify_funcgui(
        ui.reassign_widget, "Re-Assign Intensities")
    mod_grayscale: widgets.Container = modify_funcgui(
        ui.grayscale_widget, "RGB to Grayscale")
    mod_invert: widgets.Container = modify_funcgui(
        ui.invert_widget, "Invert")
    ...
    mod_bilateral: widgets.Container = modify_funcgui(
        ui.bilateral_widget, "Bilateral Filter")
    mod_gaussian: widgets.Container = modify_funcgui(
        ui.gaussian_widget, "Gaussian Blur")
    mod_nl_means: widgets.Container = modify_funcgui(
        ui.nl_means_widget, "Non-Local Means Filter")
    mod_remove_background: widgets.Container = modify_funcgui(
        ui.remove_background_widget, "Remove Background")
    mod_ring_removal: widgets.Container = modify_funcgui(
        ui.ring_removal_widget, "Ring Removal")
    mod_tv_bregman: widgets.Container = modify_funcgui(
        ui.tv_bregman_widget, "TV Bregman Filter")
    mod_tv_chambolle: widgets.Container = modify_funcgui(
        ui.tv_chambolle_widget, "TV Chambolle Filter")
    mod_wavelet: widgets.Container = modify_funcgui(
        ui.wavelet_widget, "Wavelet Filter")
    ...
    mod_manual_threshold: widgets.Container = modify_funcgui(
        ui.manual_threshold_widget, "Manual Threshold")
    mod_label: widgets.Container = modify_funcgui(
        ui.label_widget, "Label")
    mod_hist_threshold: widgets.Container = modify_funcgui(
        ui.hist_threshold_widget, "Histogram Threshold")
    mod_local_threshold: widgets.Container = modify_funcgui(
        ui.local_threshold_widget, "Local Threshold")
    mod_rand_walk: widgets.Container = modify_funcgui(
        ui.rand_walk_widget, "Random Walk")
    mod_morph_snakes: widgets.Container = modify_funcgui(
        ui.morph_snakes_widget, "Morphological Snakes")
    segmentation_container: mcw.ScrollableContainer = mcw.ScrollableContainer(
        widgets = [
            mod_manual_threshold,
            mod_label,
            mod_hist_threshold,
            mod_local_threshold,
            mod_rand_walk,
    ...
    mod_erode: widgets.Container = modify_funcgui(
        ui.erode_widget, "Erosion")
    mod_close: widgets.Container = modify_funcgui(
        ui.close_widget, "Closing")
    mod_open: widgets.Container = modify_funcgui(
        ui.open_widget, "Opening")
    mod_tophat: widgets.Container = modify_funcgui(
    ...
    mod_edge_detect: widgets.Container = modify_funcgui(
        ui.edge_detect_widget, "Edge Detection")
    mod_corner_detect: widgets.Container = modify_funcgui(
        ui.corner_detect_widget, "Corner Detection")
    mod_ridge_detect: widgets.Container = modify_funcgui(
        ui.ridge_detect_widget, "Ridge Detection")
    mod_blob_detect: widgets.Container = modify_funcgui(
        ui.blob_detect_widget, "Blob Detection")
    mod_skeleton_detect: widgets.Container = modify_funcgui(
        ui.skeleton_detect_widget, "Skeleton Detection")
    ...
    mod_line_scan: widgets.Container = modify_funcgui(
    ...
    mod_labels_2_image: widgets.Container = modify_funcgui(
        ui.labels_2_image_widget, "Labels to Image")
    mod_image_2_labels: widgets.Container = modify_funcgui(
        ui.image_2_labels_widget, "Image to Labels")
